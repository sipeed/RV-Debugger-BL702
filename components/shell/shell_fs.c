...
static int shell_readline(int fd, char *line_buf, int size)
{
    char ch;
    int index = 0;
...
int msh_exec_script(const char *cmd_line, int size)
{
    ...
    if (fd >= 0)
    {
        ...
        do
        {
            length = shell_readline(fd, line_buf, RT_CONSOLEBUF_SIZE);
            if (length > 0)
            {
                char ch = '\0';
                int index;
...
int cmd_cp(int argc, char **argv)
{
    ...
    if (argc != 3)
    {
        ...
        SHELL_PRINTF("Copy SOURCE to DEST.\n");
    }
    else
    {
        copy(argv[1], argv[2]);
...
int cmd_cat(int argc, char **argv)
{
    ...
    if (argc == 1)
    {
        ...
        SHELL_PRINTF("Concatenate FILE(s)\n");
        return 0;
    }
...
int cmd_cd(int argc, char **argv)
{
    ...
    else if (argc == 2)
    {
        if (chdir(argv[1]) != 0)
        {
            SHELL_PRINTF("No such directory: %s\n", argv[1]);
        }
    }
...
int cmd_mkdir(int argc, char **argv)
{
    if (argc == 1)
    {
        SHELL_PRINTF("Usage: mkdir [OPTION] DIRECTORY\n");
        SHELL_PRINTF("Create the DIRECTORY, if they do not already exist.\n");
    }
    else
    {
        mkdir(argv[1], 0);
...
int cmd_mkfs(int argc, char **argv)
{
    ...
    {
        SHELL_PRINTF("Usage: mkfs [-t type] device\n");
        return 0;
    }

    if (result != RT_EOK)
    {
        SHELL_PRINTF("mkfs failed, result=%d\n", result);
    }
...
int cmd_echo(int argc, char** argv)
{
    ...
    else if (argc == 3)
    {
        ...
        {
            SHELL_PRINTF("open file:%s failed!\n", argv[2]);
        }
    }
    else
    {
        SHELL_PRINTF("Usage: echo \"string\" [filename]\n");
    }
